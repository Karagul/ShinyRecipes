## Recipe 13:  Reactive data frame: That changes with changes in input data
...
server <- function(input, output) {
  ...
  movies_selected <- movies %>% select(input$selected_var) # select columns of movies
  })
  
  # Create data table using the reactive frame created in prev step.
  ...
  # Download file. Write content from the reactive frame
...
## Recipe 15:  Update Numeric Input 
...
## Recipe 16:  Reactive Values  - ISOLATE - update title
...
server <- function(input, output, session) {
  ...
  output$scatterplot <- renderPlot({
    ggplot(data = movies, aes_string(x = input$x, y = input$y, color = input$z)) +
      ...
      labs( title = isolate({input$plot_title}) )  ## Title changes only when one of the other input changes
  })
...
## Recipe 17 : eventReactive - Delayed Event based on user action
